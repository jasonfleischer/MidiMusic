...
public class MidiMusicConfig implements Serializable{
	...
	public final ArrayList<Tempo> tempos;
	public Tempo tempo;
	...
	public Sequence addSequence(int[] seq, int length){
		if(sequences.size()<30){
			Sequence sequence = new Sequence(MainActivity.getInstance().getResources().getString(R.string.playing_mode_sequence)
					+" "+seqNum++, seq, length);
			sequences.add(sequence);
			return sequence;
		}
