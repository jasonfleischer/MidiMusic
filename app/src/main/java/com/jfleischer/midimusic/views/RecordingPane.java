...
public class RecordingPane extends LinearLayout{
	...
	private static ImageButton recordBtn;
    private static ImageButton loopBtn;
	...
	public void init(){
        ...
        TextView bpmTextView = findViewById(R.id.recording_bpm);
		tempo = MainActivity.config.tempo;
		bpmTextView.setText(tempo.getBpm()+" BPM");


		timeTextView= findViewById(R.id.recording_time);
		if(masterTrack!=null){
		...
		statusTextView= findViewById(R.id.recording_status);
		recordBtn = findViewById(R.id.recording_rec_btn);
		loopBtn= findViewById(R.id.recording_loop_btn);
        ImageButton deleteTrackBtn = findViewById(R.id.recording_delete_track_btn);
		updateStatus();
			...
			public void onClick(View v) {
				if(!isLooping){//start loop
					if(masterTrack ==null || masterTrack.getDelayBeforeNextLoop()==null || masterTrack.getSoundIds().isEmpty()){
						HLog.i(getResources().getString(R.string.cannot_loop_un_complete));
						return;
					}
